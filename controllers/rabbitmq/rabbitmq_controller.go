...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, _err error) {
	...
	if instance.Spec.TLS.SecretName != "" {
		...
		hash, err := srv.ValidateCertSecret(ctx, helper, instance.Namespace)
		if err != nil {
			if k8s_errors.IsNotFound(err) {
...
`,
	...
	rabbitmqCluster := impl.NewRabbitMqCluster(rabbitmq.Cluster(instance, IPv6Enabled, fipsEnabled, topology), 5)
	rmqres, rmqerr := rabbitmqCluster.CreateOrPatch(ctx, helper)
	if rmqerr != nil {
		return rmqres, rmqerr
	}
	rabbitmqClusterInstance := rabbitmqCluster.GetRabbitMqCluster()
...
func (r *Reconciler) findObjectsForSrc(ctx context.Context, src client.Object) []reconcile.Request {
	...
	l := log.FromContext(context.Background()).WithName("Controllers").WithName("RabbitMq")
